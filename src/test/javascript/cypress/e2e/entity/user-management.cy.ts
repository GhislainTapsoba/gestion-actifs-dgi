...
describe('UserManagement e2e test', () => {
  ...
  let username: string;
  let password: string;
  const userManagementSample = { login: 'Sandra.Andre', email: 'Theophile.Mathieu@yahoo.fr' };
  ...
  describe('new UserManagement page', () => {
    ...
    it('should create an instance of UserManagement', () => {
      cy.get(`[data-cy="login"]`).type('Maud93');
      cy.get(`[data-cy="login"]`).should('have.value', 'Maud93');

      cy.get(`[data-cy="firstName"]`).type('Archange');
      cy.get(`[data-cy="firstName"]`).should('have.value', 'Archange');

      cy.get(`[data-cy="lastName"]`).type('Lacroix');
      cy.get(`[data-cy="lastName"]`).should('have.value', 'Lacroix');

      cy.get(`[data-cy="email"]`).type('Laurence_Menard@yahoo.fr');
      cy.get(`[data-cy="email"]`).should('have.value', 'Laurence_Menard@yahoo.fr');
