...
import { sampleWithRequiredData } from '../categorie-materiel.test-samples';
import { CategorieMaterielService } from '../service/categorie-materiel.service';

import { CategorieMateriel } from './categorie-materiel';

vi.useFakeTimers();

describe('CategorieMateriel Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: CategorieMateriel;
  let fixture: ComponentFixture<CategorieMateriel>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(CategorieMateriel);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 3013 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.categorieMateriels()[0]).toEqual(expect.objectContaining({ id: 3013 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 3013 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.categorieMateriels()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 3013 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to categorieMaterielService', () => {
      ...
      vi.spyOn(service, 'getCategorieMaterielIdentifier');
      const id = comp.trackId(entity);
      expect(service.getCategorieMaterielIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.categorieMaterielsParams()).toMatchObject({ sort: ['id,desc'] });
  });
