...
import { CategorieMaterielDeleteDialog } from '../delete/categorie-materiel-delete-dialog';
import { CategorieMaterielService } from '../service/categorie-materiel.service';

@Component({
  selector: 'jhi-categorie-materiel',
  templateUrl: './categorie-materiel.html',
...
export class CategorieMateriel {
  readonly categorieMateriels = signal<ICategorieMateriel[]>([]);
  ...
  readonly isLoading = this.categorieMaterielService.categorieMaterielsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.categorieMaterielService.categorieMaterielsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.categorieMateriels.set(this.fillComponentAttributesFromResponseBody([...this.categorieMaterielService.categorieMateriels()]));
    });
    effect(() => {
  ...
  trackId = (item: ICategorieMateriel): number => this.categorieMaterielService.getCategorieMaterielIdentifier(item);

  delete(categorieMateriel: ICategorieMateriel): void {
    const modalRef = this.modalService.open(CategorieMaterielDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.categorieMateriel = categorieMateriel;
  ...
  protected fillComponentAttributesFromResponseBody(data: ICategorieMateriel[]): ICategorieMateriel[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.categorieMaterielService.categorieMaterielsParams.set(queryObject);
  }
