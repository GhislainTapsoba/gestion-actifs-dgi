...
import categorieMaterielResolve from './categorie-materiel-routing-resolve.service';

describe('CategorieMateriel routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return ICategorieMateriel returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          categorieMaterielResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(categorieMaterielResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
