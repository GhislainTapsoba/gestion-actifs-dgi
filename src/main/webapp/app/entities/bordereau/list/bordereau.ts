...
import { BordereauDeleteDialog } from '../delete/bordereau-delete-dialog';
...
export class Bordereau {
  readonly bordereaus = signal<IBordereau[]>([]);
  ...
  readonly isLoading = this.bordereauService.bordereausResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.bordereauService.bordereausResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.bordereaus.set(this.fillComponentAttributesFromResponseBody([...this.bordereauService.bordereaus()]));
    });
    effect(() => {
  ...
  trackId = (item: IBordereau): number => this.bordereauService.getBordereauIdentifier(item);

  imprimer(bordereau: IBordereau): void {
    this.bordereauPdfService.imprimerBordereau(bordereau);
  }

  delete(bordereau: IBordereau): void {
    const modalRef = this.modalService.open(BordereauDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IBordereau[]): IBordereau[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.bordereauService.bordereausParams.set(queryObject);
  }
