...
import { sampleWithRequiredData } from '../transfert-actif.test-samples';

import { TransfertActif } from './transfert-actif';

vi.useFakeTimers();

describe('TransfertActif Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: TransfertActif;
  let fixture: ComponentFixture<TransfertActif>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(TransfertActif);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 4253 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.transfertActifs()[0]).toEqual(expect.objectContaining({ id: 4253 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 4253 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.transfertActifs()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 4253 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to transfertActifService', () => {
      const entity = { id: 4253 };
      vi.spyOn(service, 'getTransfertActifIdentifier');
      const id = comp.trackId(entity);
      expect(service.getTransfertActifIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.transfertActifsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.transfertActifsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });
