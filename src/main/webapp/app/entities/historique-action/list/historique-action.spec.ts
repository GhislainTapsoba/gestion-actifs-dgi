...
import { sampleWithRequiredData } from '../historique-action.test-samples';
...
describe('HistoriqueAction Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: HistoriqueAction;
  let fixture: ComponentFixture<HistoriqueAction>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(HistoriqueAction);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 7263 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.historiqueActions()[0]).toEqual(expect.objectContaining({ id: 7263 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 7263 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.historiqueActions()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 7263 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to historiqueActionService', () => {
      const entity = { id: 7263 };
      vi.spyOn(service, 'getHistoriqueActionIdentifier');
      const id = comp.trackId(entity);
      expect(service.getHistoriqueActionIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.historiqueActionsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.historiqueActionsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });
