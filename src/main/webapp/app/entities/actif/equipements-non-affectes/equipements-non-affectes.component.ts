...
@Component({
  selector: 'jhi-equipements-non-affectes',
  templateUrl: './equipements-non-affectes.component.html',
  imports: [RouterLink, FontAwesomeModule, AlertError, Alert, FormatMediumDatePipe],
})
export class EquipementsNonAffectesComponent implements OnInit {
  ...
  loadAll(): void {
    ...
    this.actifService.findEquipementsNonAffectes().subscribe({
      next: (data: IActif[]) => {
  ...
  exporterCSV(): void {
    ...
    link.setAttribute('download', `equipements_non_affectes_${new Date().toISOString().slice(0, 10)}.csv`);
