...
@Component({
  selector: 'jhi-equipements-a-reformer',
  templateUrl: './equipements-a-reformer.component.html',
  imports: [RouterLink, FontAwesomeModule, AlertError, Alert, FormatMediumDatePipe],
})
export class EquipementsAReformerComponent implements OnInit {
  ...
  loadAll(): void {
    ...
    this.actifService.findEquipementsAReformer().subscribe({
      next: (data: IActif[]) => {
  ...
  exporterCSV(): void {
    ...
    link.setAttribute('download', `equipements_a_reformer_${new Date().toISOString().slice(0, 10)}.csv`);
