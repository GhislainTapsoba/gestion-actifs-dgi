...
import { InterventionDeleteDialog } from '../delete/intervention-delete-dialog';
...
export class Intervention {
  readonly interventions = signal<IIntervention[]>([]);
  ...
  readonly isLoading = this.interventionService.interventionsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.interventionService.interventionsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.interventions.set(this.fillComponentAttributesFromResponseBody([...this.interventionService.interventions()]));
    });
    effect(() => {
  ...
  trackId = (item: IIntervention): number => this.interventionService.getInterventionIdentifier(item);

  delete(intervention: IIntervention): void {
    const modalRef = this.modalService.open(InterventionDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IIntervention[]): IIntervention[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.interventionService.interventionsParams.set(queryObject);
  }
