...
import { InventaireDeleteDialog } from '../delete/inventaire-delete-dialog';
...
export class Inventaire {
  readonly inventaires = signal<IInventaire[]>([]);
  ...
  readonly isLoading = this.inventaireService.inventairesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.inventaireService.inventairesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.inventaires.set(this.fillComponentAttributesFromResponseBody([...this.inventaireService.inventaires()]));
    });
    effect(() => {
  ...
  trackId = (item: IInventaire): number => this.inventaireService.getInventaireIdentifier(item);

  delete(inventaire: IInventaire): void {
    const modalRef = this.modalService.open(InventaireDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IInventaire[]): IInventaire[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.inventaireService.inventairesParams.set(queryObject);
  }
