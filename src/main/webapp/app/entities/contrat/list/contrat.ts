...
import { ContratDeleteDialog } from '../delete/contrat-delete-dialog';
...
export class Contrat {
  readonly contrats = signal<IContrat[]>([]);
  ...
  readonly isLoading = this.contratService.contratsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.contratService.contratsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.contrats.set(this.fillComponentAttributesFromResponseBody([...this.contratService.contrats()]));
    });
    effect(() => {
  ...
  delete(contrat: IContrat): void {
    const modalRef = this.modalService.open(ContratDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IContrat[]): IContrat[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.contratService.contratsParams.set(queryObject);
  }
