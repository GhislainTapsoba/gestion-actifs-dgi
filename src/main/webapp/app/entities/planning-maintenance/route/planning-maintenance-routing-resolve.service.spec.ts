...
import planningMaintenanceResolve from './planning-maintenance-routing-resolve.service';

describe('PlanningMaintenance routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IPlanningMaintenance returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          planningMaintenanceResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(planningMaintenanceResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
