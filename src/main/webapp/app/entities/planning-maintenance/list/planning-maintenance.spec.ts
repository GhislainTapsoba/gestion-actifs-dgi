...
import { sampleWithRequiredData } from '../planning-maintenance.test-samples';
import { PlanningMaintenanceService } from '../service/planning-maintenance.service';

import { PlanningMaintenance } from './planning-maintenance';

vi.useFakeTimers();

describe('PlanningMaintenance Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: PlanningMaintenance;
  let fixture: ComponentFixture<PlanningMaintenance>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(PlanningMaintenance);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 20630 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.planningMaintenances()[0]).toEqual(expect.objectContaining({ id: 20630 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 20630 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.planningMaintenances()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 20630 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to planningMaintenanceService', () => {
      ...
      vi.spyOn(service, 'getPlanningMaintenanceIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPlanningMaintenanceIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.planningMaintenancesParams()).toMatchObject({ sort: ['id,desc'] });
  });
