...
import { AffectationDeleteDialog } from '../delete/affectation-delete-dialog';
...
export class Affectation {
  readonly affectations = signal<IAffectation[]>([]);
  ...
  readonly isLoading = this.affectationService.affectationsResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.affectationService.affectationsResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.affectations.set(this.fillComponentAttributesFromResponseBody([...this.affectationService.affectations()]));
    });
    effect(() => {
  ...
  trackId = (item: IAffectation): number => this.affectationService.getAffectationIdentifier(item);

  delete(affectation: IAffectation): void {
    const modalRef = this.modalService.open(AffectationDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IAffectation[]): IAffectation[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.affectationService.affectationsParams.set(queryObject);
  }
