...
import { sampleWithRequiredData } from '../equipement-recensement.test-samples';
import { EquipementRecensementService } from '../service/equipement-recensement.service';

import { EquipementRecensement } from './equipement-recensement';

vi.useFakeTimers();

describe('EquipementRecensement Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: EquipementRecensement;
  let fixture: ComponentFixture<EquipementRecensement>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(EquipementRecensement);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 7919 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.equipementRecensements()[0]).toEqual(expect.objectContaining({ id: 7919 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 7919 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.equipementRecensements()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 7919 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to equipementRecensementService', () => {
      const entity = { id: 7919 };
      vi.spyOn(service, 'getEquipementRecensementIdentifier');
      const id = comp.trackId(entity);
      expect(service.getEquipementRecensementIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.equipementRecensementsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.equipementRecensementsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });
