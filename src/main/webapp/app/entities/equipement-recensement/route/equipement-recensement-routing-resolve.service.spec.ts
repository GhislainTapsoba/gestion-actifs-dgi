...
import equipementRecensementResolve from './equipement-recensement-routing-resolve.service';

describe('EquipementRecensement routing resolve service', () => {
  ...
  describe('resolve', () => {
    it('should return IEquipementRecensement returned by find', async () => {
      // GIVEN
      ...
      await new Promise<void>(resolve => {
        TestBed.runInInjectionContext(() => {
          equipementRecensementResolve(mockActivatedRouteSnapshot).subscribe({
            next(result) {
              // THEN
    ...
    it('should route to 404 page if data not found in server', async () => {
      ...
      await TestBed.runInInjectionContext(async () => {
        await expect(lastValueFrom(equipementRecensementResolve(mockActivatedRouteSnapshot))).rejects.toThrow('no elements in sequence');
        // THEN
        expect(service.find).toHaveBeenCalledWith(123);
