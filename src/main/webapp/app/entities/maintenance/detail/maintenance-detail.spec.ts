...
describe('Maintenance Management Detail Component', () => {
      ...
      providers: [
        ...
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./maintenance-detail').then(m => m.MaintenanceDetail),
              resolve: { maintenance: () => of({ id: 17111 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load maintenance on init', async () => {
      ...
      const instance = await harness.navigateByUrl('/', MaintenanceDetail);

      // THEN
      expect(instance.maintenance()).toEqual(expect.objectContaining({ id: 17111 }));
    });
  });
