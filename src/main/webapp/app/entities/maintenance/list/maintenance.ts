...
import { MaintenanceDeleteDialog } from '../delete/maintenance-delete-dialog';
...
export class Maintenance {
  ...
  readonly isLoading = this.maintenanceService.maintenancesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.maintenanceService.maintenancesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.maintenances.set(this.fillComponentAttributesFromResponseBody([...this.maintenanceService.maintenances()]));
    });
    effect(() => {
  ...
  trackId = (item: IMaintenance): number => this.maintenanceService.getMaintenanceIdentifier(item);
  ...
  delete(maintenance: IMaintenance): void {
    const modalRef = this.modalService.open(MaintenanceDeleteDialog, { size: 'lg', backdrop: 'static' });
  ...
  protected fillComponentAttributesFromResponseBody(data: IMaintenance[]): IMaintenance[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.maintenanceService.maintenancesParams.set(queryObject);
  }
