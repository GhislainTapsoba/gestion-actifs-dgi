...
import { sampleWithRequiredData } from '../recensement.test-samples';
...
describe('Recensement Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Recensement;
  let fixture: ComponentFixture<Recensement>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Recensement);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 26788 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.recensements()[0]).toEqual(expect.objectContaining({ id: 26788 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 26788 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.recensements()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 26788 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to recensementService', () => {
      ...
      vi.spyOn(service, 'getRecensementIdentifier');
      const id = comp.trackId(entity);
      expect(service.getRecensementIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.recensementsParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.recensementsParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });
