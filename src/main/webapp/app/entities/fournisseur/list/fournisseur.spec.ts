...
describe('Fournisseur Management Component', () => {
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 25678 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.fournisseurs()[0]).toEqual(expect.objectContaining({ id: 25678 }));
  });

  describe('trackId', () => {
    it('should forward to fournisseurService', () => {
      const entity = { id: 25678 };
      vi.spyOn(service, 'getFournisseurIdentifier');
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.fournisseursParams()).toMatchObject({ sort: ['id,desc'] });
  });
