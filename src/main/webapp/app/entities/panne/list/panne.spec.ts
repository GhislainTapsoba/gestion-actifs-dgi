...
import { sampleWithRequiredData } from '../panne.test-samples';
...
describe('Panne Management Component', () => {
  let httpMock: HttpTestingController;
  let comp: Panne;
  let fixture: ComponentFixture<Panne>;
  ...
  beforeEach(() => {
    ...
    fixture = TestBed.createComponent(Panne);
  ...
  it('should call load all on init', async () => {
    ...
    req.flush([{ id: 5747 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();

    // THEN
    expect(comp.isLoading()).toEqual(false);
    expect(comp.pannes()[0]).toEqual(expect.objectContaining({ id: 5747 }));
  });
  ...
  it('should cancel previous requests when loading a new page', async () => {
    ...
    req2.flush([{ id: 5747 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  it('should not fail on resource error state', async () => {
    ...
    expect(comp.pannes()).toEqual([]);

    // WHEN - second load should still work
    comp.load();
    TestBed.tick();
    const successReq = httpMock.expectOne({ method: 'GET' });
    successReq.flush([{ id: 5747 }], { headers: { link: '<http://localhost/api/foo?page=1&size=20>; rel="next"' } });
    await vi.runAllTimersAsync();
  ...
  describe('trackId', () => {
    it('should forward to panneService', () => {
      ...
      vi.spyOn(service, 'getPanneIdentifier');
      const id = comp.trackId(entity);
      expect(service.getPanneIdentifier).toHaveBeenCalledWith(entity);
  ...
  it('should calculate the sort attribute for an id', () => {
    ...
    // THEN
    expect(service.pannesParams()).toMatchObject({ sort: ['id,desc'] });
  });
  ...
  it('should calculate the filter attribute', () => {
    ...
    // THEN
    expect(service.pannesParams()).toMatchObject({ 'someId.in': ['dc4279ea-cfb9-11ec-9d64-0242ac120002'] });
  });
