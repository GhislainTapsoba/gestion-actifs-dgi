...
import { PanneDeleteDialog } from '../delete/panne-delete-dialog';
import { IPanne } from '../panne.model';
import { PanneService } from '../service/panne.service';

@Component({
  selector: 'jhi-panne',
  templateUrl: './panne.html',
  imports: [
    RouterLink,
    FontAwesomeModule,
    AlertError,
...
export class Panne {
  ...
  readonly isLoading = this.panneService.pannesResource.isLoading;
  ...
  constructor() {
    effect(() => {
      const headers = this.panneService.pannesResource.headers();
      if (headers) {
        this.fillComponentAttributesFromResponseHeader(headers);
      }
    });
    effect(() => {
      this.pannes.set(this.fillComponentAttributesFromResponseBody([...this.panneService.pannes()]));
    });
    effect(() => {
  ...
  trackId = (item: IPanne): number => this.panneService.getPanneIdentifier(item);

  delete(panne: IPanne): void {
    const modalRef = this.modalService.open(PanneDeleteDialog, { size: 'lg', backdrop: 'static' });
    modalRef.componentInstance.panne = panne;
  ...
  protected fillComponentAttributesFromResponseBody(data: IPanne[]): IPanne[] {
    return data;
  }
  ...
  protected queryBackend(): void {
    ...
    this.panneService.pannesParams.set(queryObject);
  }
