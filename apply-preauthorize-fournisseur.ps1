# Script : ajoute les annotations @PreAuthorize sur FournisseurResource.java
# Module bonus : Admin=CRUD, Technicien=R, Responsable=R, Agent=aucun acces
# A lancer depuis la racine du projet.

$path = "src\main\java\com\dgi\gestionactifs\web\rest\FournisseurResource.java"
...
$patterns = @(
    @{ Regex = '^@PostMapping\((""|"/fournisseurs")\)$';                    Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'')")' },
    @{ Regex = '^@PutMapping\((?:"/\{id\}"|"/fournisseurs/\{id\}")\)$';     Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'')")' },
    @{ Regex = '^@DeleteMapping\((?:"/\{id\}"|"/fournisseurs/\{id\}")\)$';  Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'')")' },
    @{ Regex = '^@GetMapping\((""|"/fournisseurs")\)$';                     Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'') or hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'')")' },
    @{ Regex = '^@GetMapping\((?:"/\{id\}"|"/fournisseurs/\{id\}")\)$';     Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'') or hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'')")' }
)
