# Script : ajoute les annotations @PreAuthorize sur ContratResource.java
# Module bonus : Admin=CRUD, Technicien=R, Responsable=R, Agent=aucun acces
# A lancer depuis la racine du projet.

$path = "src\main\java\com\dgi\gestionactifs\web\rest\ContratResource.java"
...
$patterns = @(
    @{ Regex = '^@PostMapping\((""|"/contrats")\)$';                    Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'')")' },
    @{ Regex = '^@PutMapping\((?:"/\{id\}"|"/contrats/\{id\}")\)$';     Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'')")' },
    @{ Regex = '^@DeleteMapping\((?:"/\{id\}"|"/contrats/\{id\}")\)$';  Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'')")' },
    @{ Regex = '^@GetMapping\((""|"/contrats")\)$';                     Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'') or hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'')")' },
    @{ Regex = '^@GetMapping\((?:"/\{id\}"|"/contrats/\{id\}")\)$';     Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'') or hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'')")' }
)
