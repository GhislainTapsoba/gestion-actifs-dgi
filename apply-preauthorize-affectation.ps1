# Script : ajoute les annotations @PreAuthorize sur AffectationResource.java
# selon la matrice : Admin=R, Technicien=CRU, Responsable=CRU, Agent=R
...
$patterns = @(
    @{ Regex = '^@PostMapping\((""|"/affectations")\)$';                    Rule = '@PreAuthorize("hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'')")' },
    @{ Regex = '^@PutMapping\((?:"/\{id\}"|"/affectations/\{id\}")\)$';     Rule = '@PreAuthorize("hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'')")' },
    @{ Regex = '^@DeleteMapping\((?:"/\{id\}"|"/affectations/\{id\}")\)$';  Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'')")' },
    @{ Regex = '^@GetMapping\((""|"/affectations")\)$';                     Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'') or hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'') or hasAuthority(''ROLE_AGENT'')")' },
    @{ Regex = '^@GetMapping\((?:"/\{id\}"|"/affectations/\{id\}")\)$';     Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'') or hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'') or hasAuthority(''ROLE_AGENT'')")' }
)
