# Script : ajoute les annotations @PreAuthorize sur TransfertResource.java
# selon la matrice : Admin=R, Technicien=CRU, Responsable=validation(RU), Agent=C (demande) + R
...
$patterns = @(
    @{ Regex = '^@PostMapping\((""|"/transferts")\)$';                    Rule = '@PreAuthorize("hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_AGENT'')")' },
    @{ Regex = '^@PutMapping\((?:"/\{id\}"|"/transferts/\{id\}")\)$';     Rule = '@PreAuthorize("hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'')")' },
    @{ Regex = '^@DeleteMapping\((?:"/\{id\}"|"/transferts/\{id\}")\)$';  Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'')")' },
    @{ Regex = '^@GetMapping\((""|"/transferts")\)$';                     Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'') or hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'') or hasAuthority(''ROLE_AGENT'')")' },
    @{ Regex = '^@GetMapping\((?:"/\{id\}"|"/transferts/\{id\}")\)$';     Rule = '@PreAuthorize("hasAuthority(''ROLE_ADMIN'') or hasAuthority(''ROLE_TECHNICIEN'') or hasAuthority(''ROLE_RESPONSABLE'') or hasAuthority(''ROLE_AGENT'')")' }
)
